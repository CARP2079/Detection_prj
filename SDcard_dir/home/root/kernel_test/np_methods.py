...
def ssd_bboxes_select_layer(predictions_layer,
    ...
    if decode:
        localizations_layer = ssd_bboxes_decode(localizations_layer, anchors_layer)  #这边返回的形状与输入保持一致，但由于操作，这边已经解码了
    ...
    if select_threshold is None or select_threshold == 0:
        ...
        sub_predictions = predictions_layer[:, :, 1:] # shape = [1, 504, 9] 0维度是背景  504=7*12*6
        idxes = np.where(sub_predictions > select_threshold) # shape = [3, N] ,其中[:,0]则表示第一个值的完整索引   这是3个数组组成的元组，3个共同定位出坐标
        classes = idxes[-1]+1 # classes = idexs 最后一个维度的索引 + 1   shape = [N]  这是因为上面的索引是对去掉0维度计算的，所以得加上。
        scores = sub_predictions[idxes]         # shape = [N]   这边会把所有的得分都定位出来，
        bboxes = localizations_layer[idxes[:-1]]  # shape = [N, 4]  这边则吧之前定位的前面的维度，即504那一个索引来索引出（N 4）的的矩阵。
...
def ssd_bboxes_select(predictions_net,
    ...
    # l_layers = []
    # l_idxes = []
    for i in range(len(predictions_net)):    #循环4次
...
def bboxes_sort(classes, scores, bboxes, top_k=400):
    ...
    idxes = np.argsort(-scores)  # np.argsort 升序排列，返回索引
...
def bboxes_clip(bbox_ref, bboxes):
    """Clip bounding boxes with respect to reference bbox. bboxes : ymin, xmin, ymax, xmax
    """
    bboxes = np.copy(bboxes)
    bboxes = np.transpose(bboxes)  # [4,N]
...
def bboxes_resize(bbox_ref, bboxes):
    """Resize bounding boxes based on a reference bounding box,    bboxes : ymin, xmin, ymax, xmax
...
def bboxes_nms(classes, scores, bboxes, nms_threshold=0.45):
    ...
    for i in range(scores.size-1):
        if keep_bboxes[i]:
            ...
            overlap = bboxes_jaccard(bboxes[i], bboxes[(i+1):])  # 第i个bbox与后面所有bbox的 IOU
