...
def ssd_anchor_one_layer(img_shape,
    ...
    xref = np.reshape(x, [-1, 1])
    yref = np.reshape(y, [-1, 1])
    xyref = np.zeros((feat_shape[0]*feat_shape[1],2,1),dtype = np.float32, order = 'C')
    xyref[:, 0, :] = xref
    xyref[:, 1, :] = yref

    whref = np.zeros((num_anchors*2,),dtype = np.float32, order = 'C')

    whref[0:num_anchors]=w
    whref[num_anchors:] = h
