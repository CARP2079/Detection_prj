...
def ssd_bboxes_select_layer(predictions_layer,
    ...
    if select_threshold is None or select_threshold == 0:
        ...
        sub_predictions = predictions_layer[:, :, 1:] # shape = [1, 5579, 9]
        idxes = np.where(sub_predictions > select_threshold) # shape = [3, N] ,其中[:,0]则表示第一个值的完整索引
        classes = idxes[-1]+1 # classes = idexs 最后一个维度的索引 + 1   shape = [N]
        scores = sub_predictions[idxes]         # shape = [N]
        bboxes = localizations_layer[idxes[:-1]]  # shape = [N, 4]
...
def ssd_bboxes_select(predictions_net,
    ...
    for i in range(len(predictions_net)):
        classes, scores, bboxes = ssd_bboxes_select_layer(
            predictions_net[i], localizations_net[i], select_threshold)
